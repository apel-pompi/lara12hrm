...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Opening Voucher', href: '/voucherheader/openingVoucher' }];
...
const submit = () => {
    const action = isEditMode.value && form.id ? route('voucherheader.openingUpdate', form.id) : route('voucherheader.openingStore');
    ...
    if (details.value.length === 0) {
        toast('Validation Error', { description: 'Please add at least one debit detail' });
        return;
    }
    if (creditDetails.value.length === 0) {
        toast('Validation Error', { description: 'Please add at least one credit detail' });
        return;
    }
    ...
    form.transform((data) => ({
        ...
        onSuccess: () => {
            setTimeout(() => {
                ...
                router.visit(route('voucherheader.opening'), { preserveScroll: true, preserveState: false });
            }, 200);
            form.reset();
            details.value = [];
            creditDetails.value = [];
...
const onEdit = async (opening: number) => {
    try {
        const response = await axios.get(route('voucherheader.openingEdit', opening));
...
const onConfirm = async (opening: number) => {
    if (!confirm('Are you sure you want to confirm this this voucher')) return;

    if (deleteForm.processing) return;

    deleteForm.put(`/voucherheader/${opening}/openingStatus`, {
        onSuccess: () => { },
        onError: () => {
...
const search = () => {
    ...
    router.get(route('voucherheader.opening'), params, {
        preserveState: false,
        preserveScroll: true,
    });
};
...
const refresh = () => {
    router.get(route('voucherheader.opening'), {}, { replace: true });
};

const perPage = ref(10);

const changePerPage = () => {
    router.get(route('voucherheader.opening'), { per_page: perPage.value }, { preserveState: false, replace: true });
};
        ...
        <div class="app-page">
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                <div class="border-b px-6 py-4">
                    <h2 class="text-lg font-semibold text-gray-800">Opening Voucher List</h2>
                    <p class="text-sm text-gray-500">Manage all Opening Voucher from here.</p>
                </div>
                <Table>
                    <TableHeader>
                        <TableRow>
                        ...
                        <TableRow>
                            ...
                            <TableCell class="text-center">
                                <div v-if="vhd.status == 'Balanced'" class="group relative inline-block">
                                    <Button class="m-[2px] cursor-pointer" size="sm" variant="outline"
            ...
            <DialogContent
                ...
                <DialogHeader class="space-y-1 border-b pb-4">
                    <DialogTitle class="text-xl font-semibold tracking-wide">
                        {{ isEditMode ? 'Edit Opening Blance' : 'Create Opening Blance' }}
                    </DialogTitle>
                    <DialogDescription class="text-sm text-gray-500">
                        {{ isEditMode ?
                            'Modify the information and click Update.' : 'Fill out the form to add a new Opening Blance.'
                        }}
                    </DialogDescription>
                </DialogHeader>
                <!-- Body -->
                            ...
                            <TableRow v-for="(row, idx) in details" :key="idx">
                                ...
                                <TableCell class="text-center">
                                    <div class="flex justify-center gap-1">
                                        ...
                                        </Button><Button variant="ghost" size="sm" class="h-7 w-7 p-0"
