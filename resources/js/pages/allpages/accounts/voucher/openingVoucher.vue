...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Opening Voucher Header', href: '/voucherheader/openingVoucher' }];
    ...
    form[method](action, {
        onSuccess: () => {
            setTimeout(() => {
                ...
                router.visit(route('voucherheader.opening'), {
                    preserveScroll: true,
                    preserveState: false,
                });
            }, 200);
...
const onConfirm = async (opening: number) => {
    ...
    deleteForm.put(`/voucherheader/${opening}/status`, {
        onSuccess: () => {},
        onError: () => {
...
<template>
    <Head title="Opening Voucher Header" />
                    ...
                    <Combobox v-model="selectedYear">
                        <div class="relative w-full md:w-48">
                            ...
                            <ComboboxOptions
                                class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border bg-white py-1 text-sm shadow-lg"
                            >
                                <div v-if="filteredYear.length === 0 && queryYear !== ''" class="px-4 py-2 text-gray-500 select-none">
                                    Nothing found.
                                </div>
