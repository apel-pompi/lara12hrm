...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Reverse Voucher', href: '/voucherheader/reverseVoucher' }];
...
const onConfirm = async (reverse: number) => {
    if (!confirm('Are you sure you want to confirm this this voucher')) return;

    if (deleteForm.processing) return;

    deleteForm.put(`/voucherheader/${reverse}/reverseStatus`, {
        onSuccess: () => { },
        onError: () => {
...
const search = () => {
    ...
    router.get(route('voucherheader.reverse'), params, {
        preserveState: false,
        preserveScroll: true,
    });
};
...
const refresh = () => {
    router.get(route('voucherheader.reverse'), {}, { replace: true });
};

const perPage = ref(10);

const changePerPage = () => {
    router.get(route('voucherheader.reverse'), { per_page: perPage.value }, { preserveState: false, replace: true });
};
        ...
        <div class="app-page">
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                <div class="border-b px-6 py-4">
                    <h2 class="text-lg font-semibold text-gray-800">Reverse Voucher List</h2>
                    <p class="text-sm text-gray-500">Manage all Reverse Voucher from here.</p>
                </div>
                <Table>
                    <TableHeader>
                        <TableRow>
            ...
            <DialogContent
                ...
                <DialogHeader class="space-y-1 border-b pb-4">
                    <DialogTitle class="text-xl font-semibold tracking-wide">
                        {{ isEditMode ? 'Edit Reverse Voucher' : 'Create Reverse Voucher' }}
                    </DialogTitle>
                    <DialogDescription class="text-sm text-gray-500">
                        {{ isEditMode ?
                            'Modify the information and click Update.' : 'Fill out the form to add a new Reverse Voucher.'
                        }}
                    </DialogDescription>
                </DialogHeader>
                <!-- Body -->
