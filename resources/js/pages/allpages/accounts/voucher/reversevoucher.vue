...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Reverse Voucher Header', href: '/voucherheader/reverseVoucher' }];
    ...
    form[method](action, {
        onSuccess: () => {
            setTimeout(() => {
                ...
                router.visit(route('voucherheader.reverse'), {
                    preserveScroll: true,
                    preserveState: false,
                });
            }, 200);
...
const onConfirm = async (reverse: number) => {
    ...
    deleteForm.put(`/voucherheader/${reverse}/status`, {
        onSuccess: () => {},
        onError: () => {
...
<template>
    <Head title="Reverse Voucher Header" />
