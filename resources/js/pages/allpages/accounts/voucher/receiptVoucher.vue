...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Receipt Voucher Header', href: '/voucherheader/receiptVoucher' }];
    ...
    form[method](action, {
        onSuccess: () => {
            setTimeout(() => {
                ...
                router.visit(route('voucherheader.receipt'), {
                    preserveScroll: true,
                    preserveState: false,
                });
            }, 200);
...
const onConfirm = async (receipt: number) => {
    ...
    deleteForm.put(`/voucherheader/${receipt}/status`, {
        onSuccess: () => {},
        onError: () => {
...
<template>
    <Head title="Receipt Voucher Header" />
