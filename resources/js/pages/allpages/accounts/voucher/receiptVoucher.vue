...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Receipt Voucher', href: '/voucherheader/receiptVoucher' }];
...
const submit = () => {
    const action = isEditMode.value && form.id ? route('voucherheader.receiptUpdate', form.id) : route('voucherheader.receiptStore');
    ...
    form.transform((data) => ({
        ...
        onSuccess: () => {
            setTimeout(() => {
                ...
                router.visit(route('voucherheader.receipt'), { preserveScroll: true, preserveState: false });
            }, 200);
            form.reset();
            details.value = [];
            creditDetails.value = [];
...
const onEdit = async (receipt: number) => {
    try {
        const response = await axios.get(route('voucherheader.receiptEdit', receipt));
...
const onConfirm = async (receipt: number) => {
    if (!confirm('Are you sure you want to confirm this this voucher')) return;

    if (deleteForm.processing) return;

    deleteForm.put(`/voucherheader/${receipt}/receiptStatus`, {
        onSuccess: () => { },
        onError: () => {
...
const search = () => {
    ...
    router.get(route('voucherheader.receipt'), params, {
        preserveState: false,
        preserveScroll: true,
    });
};
...
const refresh = () => {
    router.get(route('voucherheader.receipt'), {}, { replace: true });
};

const perPage = ref(10);

const changePerPage = () => {
    router.get(route('voucherheader.receipt'), { per_page: perPage.value }, { preserveState: false, replace: true });
};
        ...
        <div class="app-page">
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                <div class="border-b px-6 py-4">
                    <h2 class="text-lg font-semibold text-gray-800">Receipt Voucher List</h2>
                    <p class="text-sm text-gray-500">Manage all Receipt Voucher from here.</p>
                </div>
                <Table>
                    <TableHeader>
                        <TableRow>
            ...
            <DialogContent
                ...
                <DialogHeader class="space-y-1 border-b pb-4">
                    ...
                    <DialogDescription class="text-sm text-gray-500">
                        {{ isEditMode ?
                            'Modify the information and click Update.' : 'Fill out the form to add a new Receipt Voucher.'
                        }}
                    </DialogDescription>
                </DialogHeader>
                <!-- Body -->
