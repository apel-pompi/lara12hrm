...
const submit = () => {
    ...
    if (creditDetails.value.length === 0) {
        toast('Validation Error', { description: 'Please add at least one credit account' });
        return;
    }
    ...
    // Send the first credit details account for backward compatibility if backend validation complains about creditAcc
    form.creditAcc = creditDetails.value[0]?.accountcode || '';
            ...
            >
                ...
                <div class="rounded-lg border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-700/50 dark:bg-gray-800/50">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-sm font-semibold text-indigo-900 dark:text-indigo-300">Add Credit Details</h3>
                        <span
                ...
                <div v-if="creditDetails.length" class="mt-3 max-h-48 overflow-auto rounded-lg border border-gray-200 dark:border-gray-700">
                    <Table>
                        <TableHeader
                            ><TableRow
                                ><TableHead class="w-12">SL</TableHead><TableHead>Credit Account</TableHead><TableHead>Notes</TableHead
                                ><TableHead class="text-right">Amount</TableHead><TableHead class="w-24 text-center">Action</TableHead></TableRow
                            ></TableHeader
                        >
                        <TableBody>
                    ...
                    <div
                        class="flex items-center justify-between border-t border-indigo-200 bg-indigo-50 px-4 py-3 dark:border-indigo-800 dark:bg-indigo-900/30"
                    >
                        <span class="text-sm font-semibold text-indigo-700 dark:text-indigo-300">Total Credit Amount</span>
                        <span class="text-base font-bold text-indigo-800 dark:text-indigo-200">৳ {{ creditDetailsTotal.toLocaleString() }}</span>
                    </div>
                </div>
                <div
                    v-else
