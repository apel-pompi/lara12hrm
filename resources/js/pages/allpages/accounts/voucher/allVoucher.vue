...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <Dialog v-model:open="showDialog">
            <DialogContent
                class="max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-xl dark:border-gray-800 dark:bg-gray-900">
                <!-- Header -->
                <DialogHeader class="space-y-1 border-b pb-4">
                    ...
                    <DialogDescription class="text-sm text-gray-500">
                        {{ isEditMode ?
                            'Modify the information and click Update.' : 'Fill out the form to add a new All Voucher List.'
                        }}
                    </DialogDescription>
                </DialogHeader>
                <!-- Body -->
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <!-- Debit Account -->
                    <div>
                        <Label for="debitAcc" class="text-sm font-medium">Debit Account<span
                                class="text-red-500">*</span></Label>
                        <Combobox v-model="selectedDebit">
                            <div class="relative">
                                ...
                                <ComboboxOptions
                                    class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border border-gray-200 bg-white py-1 text-sm shadow-lg ring-1 ring-black/5 focus:outline-none dark:border-gray-700 dark:bg-gray-900">
                                    <div v-if="filteredDebit.length === 0 && queryDebit !== ''"
                                        class="cursor-default px-4 py-2 text-gray-500 select-none">
                                        Nothing found.
                                    </div>

                                    <ComboboxOption v-for="n in filteredDebit" :key="n.id" :value="n"
                                        class="ui-active:bg-indigo-600 ui-active:text-white ui-selected:font-medium relative cursor-pointer py-2 pr-4 pl-10 select-none"
                                        v-slot="{ selected }">
                                        <span :class="['block truncate', selected ? 'font-medium' : 'font-normal']"> {{
                                            n.description }}</span>
                    ...
                    <!-- Debit Amount -->
                    <div>
                        <Label for="debitAmt" class="text-sm font-medium">Debit Amount<span
                                class="text-red-500">*</span></Label>
                        <Input type="text" id="debitAmt" v-model="form.debitAmt" placeholder="Enter Debit Amount"
                            class="mt-1 w-full" />
                        <p v-if="form.errors.debitAmt" class="mt-1 text-sm text-red-600">{{ form.errors.debitAmt }}</p>
                    </div>
                    <!-- Credit Account -->
                    <div>
                        <Label for="creditAcc" class="text-sm font-medium">Credit Account<span
                                class="text-red-500">*</span></Label>
                        <Combobox v-model="selectedCredit">
                            <div class="relative">
                                ...
                                <ComboboxOptions
                                    class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border border-gray-200 bg-white py-1 text-sm shadow-lg ring-1 ring-black/5 focus:outline-none dark:border-gray-700 dark:bg-gray-900">
                                    <div v-if="filteredCredit.length === 0 && queryCredit !== ''"
                                        class="cursor-default px-4 py-2 text-gray-500 select-none">
                                        Nothing found.
                                    </div>

                                    <ComboboxOption v-for="n in filteredCredit" :key="n.id" :value="n"
                                        class="ui-active:bg-indigo-600 ui-active:text-white ui-selected:font-medium relative cursor-pointer py-2 pr-4 pl-10 select-none"
                                        v-slot="{ selected }">
                                        <span :class="['block truncate', selected ? 'font-medium' : 'font-normal']"> {{
                                            n.description }}</span>
                        ...
                        <p v-if="form.errors.creditAcc" class="mt-1 text-sm text-red-600">{{ form.errors.creditAcc }}
                        </p>
                    </div>

                    <!-- Credit Amount -->
                    <div>
                        <Label for="creditAmt" class="text-sm font-medium">Credit Amount<span
                                class="text-red-500">*</span></Label>
                        <Input type="text" id="creditAmt" v-model="form.creditAmt" placeholder="Ex: -1000"
                            class="mt-1 w-full" />
                        <p class="text-xs text-red-500">add minus sign(-) before numeric digit(s)</p>
                        <p v-if="form.errors.creditAmt" class="mt-1 text-sm text-red-600">{{ form.errors.creditAmt }}
                        </p>
                    </div>
                </div>
