...
const onEdit = async (id: number) => {
    try {
        const res = await fetch(`/groupOne/${id}/edit`);

        if (!res.ok) {
            toast.error('Server error while fetching group one details.');
            return;
        }
...
const submit = () => {
    const action = isEditMode.value && form.id ? route('GroupOne.update', form.id) : route('GroupOne.store');
    ...
    form[method](action, {
        onSuccess: () => {
            setTimeout(() => {
                ...
                router.visit(route('accsetting.GroupOne'), {
                    preserveScroll: true,
                    preserveState: false,
                });
            }, 200);
...
const onDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this group one?')) return;

    if (deleteForm.processing) return;

    deleteForm.delete(`/groupOne/show/${id}`, {
        onSuccess: () => {
            const flash = usePage().props.flash;
            if (flash?.success) {
                toast('success', {
                ...
                <DialogContent class="max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-xl dark:border-gray-800 dark:bg-gray-900">
                    ...
                    <DialogHeader class="space-y-1 border-b pb-4">
                        <DialogTitle class="text-xl font-semibold tracking-wide">
                            {{ isEditMode ? 'Edit Group One' : 'Create Group One' }}
                        </DialogTitle>
                        <DialogDescription class="text-sm text-gray-500">
                            {{ isEditMode ? 'Modify the information and click Update.' : 'Fill out the form to add a new Group One.' }}
                        </DialogDescription>
                    </DialogHeader>
