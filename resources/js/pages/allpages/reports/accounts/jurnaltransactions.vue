...
const props = defineProps<{
    ...
    transaction: Array<{ id: number; trncode: string }>;
}>();
...
const queryTransaction = ref('');
const filteredTransaction = computed(() => {
    if (queryTransaction.value === '') return props.transaction;

    return props.transaction.filter((n) => n.trncode && n.trncode.toLowerCase().includes(queryTransaction.value.toLowerCase()));
});
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Jurnal Transactions" />
        <AccountsReportsLayout>
            <div class="mx-auto max-w-md space-y-6 rounded-lg bg-white p-6 shadow-md dark:bg-gray-800">
                <!-- Title -->
                <h2 class="text-center text-xl font-semibold text-gray-800 dark:text-gray-100">Jurnal Transactions Reports</h2>
                <div class="space-y-2">
                    <Combobox v-model="selectedTransaction">
                        <div class="relative">
                            <ComboboxInput
                                class="w-full rounded-md border px-3 py-2 text-sm"
                                placeholder="All Transaction"
                                @input="queryTransaction = $event.target.value"
                                :display-value="(c) => c?.trncode ?? ''"
                            />
                            ...
                            >
                                <div v-if="filteredTransaction.length === 0 && queryTransaction !== ''" class="px-4 py-2 text-gray-500 select-none">
                                    Nothing found.
                                </div>

                                <ComboboxOption
                                    v-for="one in filteredTransaction"
                                    :key="one.trncode"
                                    :value="one"
