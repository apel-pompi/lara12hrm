...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Academic Requirements', href: '/academics' }];

const props = defineProps<{
    academic: Paginated<Academic>;
    academicFilter: { name: string };
    filters: { name: string };
}>();

const data = props.academic;
...
const onEdit = async (id: number) => {
    try {
        const res = await fetch(`/academics/${id}/edit`);

        if (!res.ok) {
            toast.error('Server error while fetching academic details.');
            return;
        }
...
const submit = () => {
    const action = isEditMode.value && form.id ? route('academic.update', form.id) : route('academic.store');
    const method = isEditMode.value ? 'put' : 'post';

    form[method](action, {
        onSuccess: () => {
            toast('Success', {
                description: `Academic Name ${isEditMode.value ? 'updated' : 'created'} successfully`,
            });
            setTimeout(() => {
                showDialog.value = false;
                form.reset();
                router.visit(route('academic.index'), {
                    only: ['academics'],
                    preserveScroll: true,
                    preserveState: false,
                });
...
const toggleStatus = (academic: Academic) => {
    const newStatus = !Boolean(academic.active); // boolean
    router.put(
        route('academic.updateStatus', academic.id),
        { active: newStatus ? 1 : 0 }, // server expects number
        {
            preserveState: true,
            onSuccess: () => {
                academic.active = newStatus ? 1 : 0; // local update (number)
                toast.success('academic  status update');
            },
        },
    );
};
...
const onDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this academic?')) return;

    if (deleteForm.processing) return;

    deleteForm.delete(`/academics/show/${id}`, {
        onSuccess: () => {
            toast.success('Academic deleted successfully');
        },
        onError: () => {
...
// Filtered lists
const filteredName = computed(() => (queryName.value === '' ? props.academicFilter : props.academicFilter.filter((n) => n.name)));
const search = () => {
    const params: Record<string, any> = {};

    if (selecteName.value) params.name = selecteName.value.name;

    router.get(route('academic.index'), params, {
        preserveState: false,
        preserveScroll: true,
    });
};
...
const refresh = () => {
    router.get(route('academic.index'), {}, { replace: true });
};

const perPage = ref(10);

const changePerPage = () => {
    router.get(route('academic.index'), { per_page: perPage.value }, { preserveState: false, replace: true });
};
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <AgencyLayout>
            <div class="border-sidebar-border/70 dark:border-sidebar-border relative min-h-screen flex-1 border px-4 md:min-h-min">
                ...
                <div class="rounded-md border">
                    <Table>
                        ...
                        <TableBody>
                            <TableRow v-for="(acc, index) in data.data" :key="acc.id ?? index">
                                <TableCell>{{ acc.name }}</TableCell>
                                <TableCell>{{ acc.adddate }}</TableCell>
                                <TableCell>{{ acc.user.name }}</TableCell>
                                <TableCell>
                                    <Switch v-model="acc.active" :checked-value="1" :unchecked-value="0" @click="toggleStatus(acc)"> </Switch>
                                </TableCell>

                                <TableCell class="text-right">
                                    <Button size="sm" variant="outline" @click="onEdit(acc.id)"><SquarePen></SquarePen></Button>
                                    <Button size="sm" variant="outline" @click="onDelete(acc.id)"><Trash></Trash></Button>
                                </TableCell>
                            </TableRow>
                        </TableBody>
                    </Table>
                ...
                <div class="flex flex-col items-center justify-between space-y-3 py-4 md:flex-row md:space-y-0">
                    <div class="text-muted-foreground flex flex-1 items-center space-x-2 text-sm">
                        ...
                        <span>Showing {{ academic.from }} to {{ academic.to }} of {{ academic.total }} results</span>
                    </div>
                    <div class="space-x-2">
                        <Button
            ...
            <Dialog v-model:open="showDialog">
                <DialogContent class="max-w-lg rounded-2xl shadow-lg sm:max-w-xl">
                    <!-- Header -->
                    <DialogHeader class="border-b pb-3">
                        <DialogTitle class="text-lg font-semibold">
                            {{ isEditMode ? 'Edit Academic' : 'Create Academic' }}
                        </DialogTitle>
                        <DialogDescription class="text-sm text-gray-500">
                            {{
                                isEditMode
                                    ? 'Update the academic details and click save.'
                                    : 'Fill in the details below to create a new academic.'
                            }}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div class="grid gap-6 py-4">
                        ...
                        <div class="grid gap-2">
                            ...
                            <Input id="name" placeholder="Enter academic name" v-model="form.name" autofocus class="max-w-sm" />
