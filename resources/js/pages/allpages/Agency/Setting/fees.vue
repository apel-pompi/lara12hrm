...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Fees', href: '/fees' }];
...
const onEdit = async (id: number) => {
    try {
        const res = await fetch(`/fees/${id}/edit`);

        if (!res.ok) {
            toast.error('Server error while fetching fees details.');
            return;
        }
...
const submit = () => {
    const action = isEditMode.value && form.id ? route('fees.update', form.id) : route('fees.store');
    const method = isEditMode.value ? 'put' : 'post';

    form[method](action, {
        onSuccess: () => {
            toast('Success', {
                description: `Fees Name ${isEditMode.value ? 'updated' : 'created'} successfully`,
            });
            setTimeout(() => {
...
const toggleStatus = (fees: Fees) => {
    const newStatus = !Boolean(fees.active); // boolean
    router.put(
        route('fees.updateStatus', fees.id),
        { active: newStatus ? 1 : 0 }, // server expects number
        {
            preserveState: true,
            onSuccess: () => {
                fees.active = newStatus ? 1 : 0; // local update (number)
                toast.success('Fees  status update');
            },
        },
    );
};
...
const onDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this fees?')) return;

    if (deleteForm.processing) return;

    deleteForm.delete(`/fees/show/${id}`, {
        onSuccess: () => {
...
const changePerPage = () => {
    router.get(route('fees.index'), { per_page: perPage.value }, { preserveState: false, replace: true });
};
            ...
            <div class="border-sidebar-border/70 dark:border-sidebar-border relative min-h-screen flex-1 border px-4 md:min-h-min">
                        ...
                        <TableBody>
                            <TableRow v-for="(fee, index) in data.data" :key="fee.id ?? index">
                                ...
                                <TableCell class="text-right">
                                    <Button size="sm" variant="outline" @click="onEdit(fee.id)"><SquarePen></SquarePen></Button>
                                    <Button size="sm" variant="outline" @click="onDelete(fee.id)"><Trash></Trash></Button>
                                </TableCell>
                            </TableRow>
                        </TableBody>
                    </Table>
                ...
                <div class="flex flex-col items-center justify-between space-y-3 py-4 md:flex-row md:space-y-0">
                    <div class="text-muted-foreground flex flex-1 items-center space-x-2 text-sm">
                        ...
                        <span>Showing {{ fees.from }} to {{ fees.to }} of {{ fees.total }} results</span>
                    </div>
                    <div class="space-x-2">
                        <Button
