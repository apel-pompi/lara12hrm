...
<template>
    <div
        class="cursor-pointer border-b border-slate-100 transition-all duration-200"
            ...
            <div class="flex gap-3">
                ...
                <div class="relative shrink-0">
                    <div class="h-12 w-12 overflow-hidden rounded-full bg-slate-200">
                        <img v-if="conversation.profile_picture" :src="conversation.profile_picture" class="h-full w-full object-cover" />
                    ...
                    <span class="absolute right-0 bottom-0 h-3 w-3 rounded-full border-2 border-white" :class="platformDot(conversation.platform)" />
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-3 flex items-center justify-between">
                        <div class="flex min-w-0 items-center gap-1">
                            <PaperClipIcon v-if="conversation.last_message_type != 'text'" class="h-4 w-4 shrink-0 text-slate-400" />

                            <template v-if="conversation.last_message_direction == 'outbound'">
                                <CheckIcon v-if="conversation.last_message_status == 'sent'" class="h-4 w-4 shrink-0 text-slate-400" />

                                <CheckIcon v-else-if="conversation.last_message_status == 'delivered'" class="h-4 w-4 shrink-0 text-slate-500" />

                                <CheckBadgeIcon v-else-if="conversation.last_message_status == 'read'" class="h-4 w-4 shrink-0 text-blue-500" />

                                <span class="shrink-0 text-xs font-medium text-slate-600"> You: </span>
                            </template>

                            <span v-if="conversation.typing" class="truncate text-sm text-green-600 italic"> Typing... </span>
