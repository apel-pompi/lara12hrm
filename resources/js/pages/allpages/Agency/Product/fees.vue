...
<template>
    <ProductLayout :product="props.product">
                ...
                <div class="mx-auto max-w-4xl space-y-6 p-6">
                    ...
                    <div v-for="fee in props.feesDt" :key="fee.id" class="rounded-lg border p-6 shadow-sm">
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500">
                                    Valid For <br />
                                    <Badge variant="outline" size="sm" v-for="country in fee.country_names"
                                        :key="country">{{ country }}</Badge>
                                </p>
                                <p class="mt-1 text-sm text-gray-500">
                            ...
                            <div class="text-right">
                                ...
                                <div v-for="detailFees in fee.details" :key="detailFees" class="mt-1">
                                    ...
                                    <p class="text-sm">{{ detailFees.insqty }} {{ fee.installment.name }} @ {{
                                        detailFees.amount }}</p>
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            <!-- Buttons on the left -->
                            <div class="space-x-2">
                                <Button variant="outline" size="sm" @click="editDialog(fee)">
                                    <SquarePen class="mr-1" /> Edit
                                </Button>
                                <Button class="m-[2px]" size="sm" variant="outline"
                                    @click="onDelete(fee.id, props.product.id)">
                                    <Trash></Trash>
                                </Button>
                            </div>
        ...
        <Dialog v-model:open="showDialog">
            <DialogContent
                class="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-y-auto sm:max-w-3xl md:max-w-4xl lg:max-w-5xl">
                <DialogHeader>
                    <DialogTitle>{{ isEditMode ? 'Edit Fee Option' : 'Add New Fee Option' }}</DialogTitle>
                    <DialogDescription> Make changes to your fee option here. Click save when you're done.
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        ...
                        <div class="w-full">
                            ...
                            <div class="relative w-full">
                                <div class="flex min-h-[40px] cursor-text flex-wrap items-center gap-1 rounded-lg border px-2 py-1 shadow-sm"
                                    @click="$refs.inputCountry.focus()">
                                    <span v-for="p in selectedCountry" :key="p.id"
                            ...
                            <Combobox v-model="selectedInsType">
                                <div class="relative">
                                    <ComboboxInput
                                        ...
                                        placeholder="Search ins" @input="queryInsType = $event.target.value"
                                    ...
                                    <ComboboxOptions
                                        ...
                                        <div v-if="filteredInsType.length === 0 && queryInsType !== ''"
                                            class="px-4 py-2 text-gray-500">
                                            Nothing found.
                                        </div>
                                        <ComboboxOption v-for="partner in filteredInsType" :key="partner.id"
                                            :value="partner"
                    ...
                    <div v-for="(row, index) in form.rows" :key="index" class="mb-4 rounded-xl border bg-gray-50 p-4">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-5">
                            <div>
                                ...
                                <Combobox v-model="row.fees">
                                    <div class="relative">
                                        <ComboboxInput
                                            class="w-full rounded-lg border-gray-300 bg-white py-2 pr-10 pl-3 text-sm shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                                            placeholder="Search fees" @input="row.query = $event.target.value"
                                            :display-value="(c) => (c ? c.name : '')" />
                                        <ComboboxButton class="absolute inset-y-0 right-0 flex items-center pr-3">
                                            <ChevronUpDownIcon class="h-5 w-5 text-gray-400" />
                                        </ComboboxButton>
                                        <ComboboxOptions
                                            class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-lg border bg-white py-1 text-sm shadow-lg">
                                            <div v-if="filteredFeesType(row.query).length === 0 && row.query !== ''"
                                                class="px-4 py-2 text-gray-500">
                                                Nothing found.
                                            </div>
                                            <ComboboxOption v-for="fees in filteredFeesType(row.query)" :key="fees.id"
                                                :value="fees"
                                                class="cursor-pointer px-3 py-2 hover:bg-indigo-600 hover:text-white">
                                                {{ fees.name }}
                                            </ComboboxOption>
                            ...
                            <div>
                                ...
                                <Input v-model.number="row.ins_amount" class="w-full" type="number"
                                    placeholder="0.00" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <Input v-model.number="row.totalfees" :value="(row.ins_amount * row.insqty).toFixed(2)"
                                    class="w-full" type="number" readonly />
                            </div>
                            <div>
                        ...
                        <div class="mt-2 flex justify-end">
                            <Button variant="default" size="sm" @click="removeRow(index)" v-if="form.rows.length > 1">
                                <Trash></Trash>
                            </Button>
                        </div>
                    </div>
