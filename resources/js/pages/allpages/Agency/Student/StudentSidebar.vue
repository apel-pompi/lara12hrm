    ...
    <aside class="w-full lg:w-72 xl:w-80 flex-shrink-0">
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <div class="flex flex-col items-center px-5 pt-6 pb-4 text-center">
                ...
                <div class="relative">
                    <div
                        class="flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-indigo-400 to-indigo-600 text-2xl font-bold text-white shadow-lg ring-4 ring-indigo-100 dark:ring-indigo-900/50">
                        {{ (props.student.fname?.charAt(0) ?? '').toUpperCase() }}{{ (props.student.lname?.charAt(0) ??
                            '').toUpperCase() }}
                    </div>
                    <!-- Status Badge on avatar -->
                    <div class="absolute -bottom-1 -right-1">
                        <Badge size="sm"
                            :class="[getStatusText(props.student.status).color, 'text-[10px] px-1.5 py-0.5 shadow-sm']">
                ...
                <div class="mt-3 flex items-center gap-1">
                    <button v-for="btn in ratingButtons" :key="btn.value" @click="updateRate(btn.value)"
                        :title="btn.label"
                ...
                <!-- Action Icon Buttons -->
                <div
                    class="mt-3 flex items-center justify-center gap-2 border-t border-gray-100 pt-3 dark:border-gray-700/50 w-full">
                    <!-- Compose SMS -->
                    ...
                    <!-- Archive / Restore -->
                    <div class="group relative">
                        <button v-if="props.student.status !== 4" @click="updateArchive(props.student.id, 4)"
                        ...
                        <button v-else @click="updateArchive(props.student.id, 1)"
                    ...
                    <!-- Assignee / Transfer -->
                    <div class="group relative">
                        <button v-if="props.student.status <= 1" @click="showDailogCreate"
                            class="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 transition-all hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/30 dark:hover:text-green-400"
                            title="Assign user">
                            <UserCheck class="h-4 w-4" />
                        </button>
                        <button v-else @click="updateTransfer(props.student.id)"
            ...
            <div class="border-t border-gray-100 px-5 py-4 dark:border-gray-700/50">
                ...
                <!-- OnBoard notice -->
                <div v-if="props.student.status == 2"
                ...
                <div v-if="props.student.student_id && props.student.status == 2" class="mt-3">
                    <button @click="updateonBoard(props.student.id)"
