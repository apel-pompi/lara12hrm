...
// Debounced search
const searchStudents = (type: 'name' | 'phone' | 'assain' | 'date' | 'status', query: string) => {
        ...
        <div class="app-page">
            ...
            <div
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7">
                    ...
                    <!-- Status -->
                    <Combobox v-model="selectedStatus" class="w-full">
                        <div class="relative w-full">
                            <ComboboxInput class="input" placeholder="Status..." :display-value="(s) => s?.label ?? ''"
                                @input="($e) => searchStatus($e.target.value)" @focus="() => searchStatus('')" />

                            <ComboboxButton class="icon-btn" @click="() => searchStatus('')">
                                <ChevronUpDownIcon class="icon" />
                            </ComboboxButton>

                            <ComboboxOptions class="dropdown">
                                <div v-if="statusResults.length === 0" class="empty">No data</div>

                                <ComboboxOption v-for="s in statusResults" :key="s.value" :value="s" class="option">
                                    {{ s.label }}
                                </ComboboxOption>
                            </ComboboxOptions>
                        </div>
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                <div class="border-b px-6 py-4">
                    <h2 class="text-lg font-semibold text-gray-800">All Student list</h2>
                    <p class="text-sm text-gray-500">Manage all Student from here.</p>
                </div>
                <Table class="w-full min-w-225 text-sm">
                    <TableHeader>
                        <TableRow class="bg-gray-100 hover:bg-gray-200">
                            <TableHead class="th">Student Name</TableHead>
                            <TableHead class="th">Phone</TableHead>
                            <TableHead class="th">Gender</TableHead>
                            <TableHead class="th">Source</TableHead>
                            <TableHead class="th">Stage</TableHead>
