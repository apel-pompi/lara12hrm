...
// Selection state (to allow transferring selected rows)
...
const toggleAll = () => {
    ...
    else selectedIds.value = props.student.data.map((s) => s.id);
};
...
const fetchStudents = async (type: 'name' | 'phone' | 'assain' | 'date', query: string = '') => {
    try {
        const res = await axios.get<Student[]>('/student/SearchPending', {
            params: { type, q: query },
        });
...
const submitTransfer = () => {
    ...
    if (transferMode.value === 'all') {
        const ids = props.student.data.map((s) => s.id);
        if (ids.length === 0) {
            transferError.value = 'No student to transfer.';
            return;
        }
        transferForm.student_ids = ids;
        transferForm.transfer_all = false;
    } else {
        if (selectedIds.value.length === 0) {
            transferError.value = 'Please select students to transfer.';
            return;
        }
    ...
    transferForm.post(route('student.transferInactiveLeads'), {
        ...
        onSuccess: () => {
            ...
            transferSuccess.value = 'Student(s) transferred successfully.';
            setTimeout(() => refresh(), 800);
        },
        onError: () => {
            transferError.value = 'Failed to transfer students.';
        },
    });
};
</script>
        ...
        >
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                <div class="border-b px-6 py-4">
                    <h2 class="text-lg font-semibold text-gray-800">Pending Student list</h2>
                    <p class="text-sm text-gray-500">Manage all pending Student from here.</p>
                </div>
