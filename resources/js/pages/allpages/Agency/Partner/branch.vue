...
const submit = () => {
    const action = isEditMode.value && form.id ? route('partnerbranch.update', form.id) : route('PartnerActivities.branchStore', props.partner.id);
    ...
    form[method](action, {
        onSuccess: () => {
            ...
            setTimeout(() => {
                ...
                form.reset();
                router.visit(route('PartnerActivities.branch', props.partner.id), {
...
const onDelete = async (id: number, partnerId: number) => {
    if (!confirm('Are you sure you want to delete this partner branch?')) return;

    if (deleteForm.processing) return;

    deleteForm.delete(`/partner/activities/${partnerId}/branch/show/${id}`, {
        onSuccess: () => {
            toast.success('Partner Branch deleted successfully');
        },
        onError: () => {
...
<template>
    <PartnerLayout :partner="props.partner">
        <div class="space-y-4">
            <div>
                <Button variant="outline" size="sm" @click="showDailogCreate"><Plus></Plus>Add Branch</Button>
            </div>
            <Table>
                <TableHeader>
                    <TableRow>
                ...
                <TableBody>
                    <TableRow v-for="(branch, index) in props.partnerbranch" :key="branch.id ?? index">
                        <TableCell>{{ branch.branch_name }}</TableCell>
                        <TableCell>{{ branch.branch_email }}</TableCell>
                        <TableCell>{{ branch.states?.country.name ?? '-' }}</TableCell>
                        <TableCell>{{ branch.states?.name ?? '-' }}</TableCell>
                        <TableCell>{{ branch.citys?.name ?? '-' }}</TableCell>

                        <TableCell>{{ branch.user.name }}</TableCell>
                        <TableCell class="text-right">
                            <Button class="m-[2px]" size="sm" variant="outline" @click="onDelete(branch.id,props.partner.id)"><Trash></Trash></Button>
                        </TableCell>
                    </TableRow>
                </TableBody>
            </Table>
        ...
        <Dialog v-model:open="showDialog">
            <DialogContent class="w-full max-w-3xl rounded-2xl shadow-lg">
                ...
                <div class="mt-5 grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <!-- Email -->
                    <div>
                        <Label class="block pb-2 text-sm font-medium text-gray-700">Email <span class="text-red-500">*</span></Label>
                    ...
                    <!-- City -->
                    <div>
                        <Label class="block pb-2 text-sm font-medium text-gray-700">City</Label>
                    ...
                    <!-- Phone -->
                    <div>
                        <Label class="block pb-2 text-sm font-medium text-gray-700">Phone Code</Label>
                        <div class="mt-1 flex">
                            <!-- Country Code -->
