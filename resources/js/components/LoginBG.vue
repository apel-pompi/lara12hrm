...
// Emit
const emit = defineEmits(['loginbg']);
...
const imageSelected = (e: Event) => {
    ...
    oversizedImage.value = file.size > 1 * 1024 * 1024; // 3MB
...
const revertImageChange = () => {
    ...
    showRevertBtn.value = false;
    emit('loginbg', null); // clear selected image
};
</script>
...
<template>
    <div>
        ...
        >
            <img :src="preview || '/storage/company/default.png'" class="h-full w-full object-cover object-center" alt="Preview" />
            <Button
        ...
        <Input @input="imageSelected" type="file" name="login-image" id="login-image" accept="image/*" hidden :disabled="disabled" />
    </div>
</template>
