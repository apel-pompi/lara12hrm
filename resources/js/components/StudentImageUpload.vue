...
// Reactive state
const currentImage = props.Image ? `/storage/student/${props.Image}` : null;
...
watch(
  () => props.Image,
  (newVal) => {
    preview.value = newVal ? `/storage/student/${newVal}` : null;
...
const revertImageChange = () => {
  preview.value = props.Image ? `/storage/student/${props.Image}` : null;
...
<template>
  <div>
    ...
    >
      <img
        :src="preview || '/storage/student/default.png'"
