...
class StudentArchive
{
    public function get(array $queryParams = [])
    {
        ...
        if ($roles->contains('superadmin')) {
            $queryBuilder = ModelsStudent::with(['user', 'assainuser', 'source', 'country'])->where('status',4)->orderBy('id', 'DESC');
        }elseif($roles->contains('Admin')){
            $queryBuilder = ModelsStudent::with(['user', 'assainuser', 'source', 'country'])->where('status',4)->orderBy('id', 'DESC');
        }elseif($roles->contains('Manager')){
            $queryBuilder = ModelsStudent::with(['user', 'assainuser', 'source', 'country'])->where('status',4)->orderBy('id', 'DESC');
        }else{
            $queryBuilder = ModelsStudent::with(['user', 'assainuser', 'source', 'country'])->where('assain_user',Auth::id())->where('status',4)->orderBy('id', 'DESC');
        }
